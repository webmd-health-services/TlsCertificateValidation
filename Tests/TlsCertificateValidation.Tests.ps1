...
Describe ('TlsCertificateValidation.help topic') {
    It 'should have one' {
        Init
        GivenModuleImported
        ThenHelpTopic 'about_TlsCertificateValidation' -Exists
    }
}

Describe ('TlsCertificateValidation.command verbs') {
...
Describe ('TlsCertificateValidation.command help topics') {
    It 'should have a help topic for each command' {
        Init
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'TlsCertificateValidation' -CommandType Function,Cmdlet,Filter))
        {
